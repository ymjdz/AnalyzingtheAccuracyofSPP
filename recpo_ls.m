...
for iter = 1:no_iterations
    ...
    for i = 1:m        
        k = col_Eph(i);%find_eph找到的星历的列数
        tx_RAW = time - obs(i)/v_light;
        t0c = Eph(21,k);
        dt = check_t(tx_RAW-t0c);
        tcorr = (Eph(2,k)*dt + Eph(20,k))*dt + Eph(19,k);%卫星钟差dts=α0+α1（t-toc）+α2（t-toc）^2
        tx_GPS = tx_RAW-tcorr;
        dt = check_t(tx_GPS-t0c);
        tcorr = (Eph(2,k)*dt + Eph(20,k))*dt + Eph(19,k);
        tx_GPS = tx_RAW-tcorr;%信号发射时刻
        ...
        if iter == 1
            ...
            Rot_X = e_r_corr(traveltime,X);%地球自转改正
            rho2 = (Rot_X(1)-pos(1))^2+(Rot_X(2)-pos(2))^2+(Rot_X(3)-pos(3))^2;          
            [az,el,dist] = topocent(pos(1:3,:),Rot_X-pos(1:3,:)); %转换为站心坐标系                                                           
            if iter == no_iterations, El(i) = el; end%当迭代到最后一次时，将高度角赋给返回值输出
        ...
        omc = [omc; obs(i)-norm(Rot_X-pos(1:3),'fro')-pos(4)+v_light*tcorr-trop]; %卫星在观测方程中的余数项
